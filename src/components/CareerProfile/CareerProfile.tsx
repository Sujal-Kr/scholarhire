...
const CareerProfile = () => {
  ...
  const [profiles, setProfiles] = useState<Array<{ company: string, position: string, years: string }>>([
    { company: 'Company A', position: 'Software Engineer', years: '2015-2018' },
    { company: 'Company B', position: 'Senior Developer', years: '2018-2021' },
  ]);

  const [formData, setFormData] = useState({ company: '', position: '', years: '' });
  ...
  const handleSave = () => {
    ...
    setFormData({ company: '', position: '', years: '' });
    setActive(false);
        ...
        {profiles.map((profile, index) => (
          <div key={index} className='flex items-center justify-between'>
            <div className='text-sm md:text-base'>
              <p className='text-slate-700 font-semibold'>{profile.company}</p>
              <p className='text-slate-500'>{profile.position}</p>
              <p className='text-slate-400'>{profile.years}</p>
            </div>
      ...
      >
        <div className='bg-white rounded-md p-4 md:p-8 w-full max-w-2xl h-full md:h-fit'>
          <div className='flex items-center gap-3'>
            ...
            <h3>{currentEditIndex !== null ? 'Edit Career Profile' : 'Add Career Profile'}</h3>
          </div>
          <p className='text-xs md:text-sm my-4 text-slate-500'>
            {currentEditIndex !== null ? 'Edit your career profile details below.' : 'Add new career profile details below.'}
          </p>
