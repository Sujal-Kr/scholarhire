...
const Accomplishments = () => {
  ...
  const [accomplishments, setAccomplishments] = useState<Array<{ title: string, description: string, date: string }>>([
        ...
        {accomplishments.map((accomplishment, index) => (
          <div key={index} className='flex items-center justify-between'>
            <div className='text-sm md:text-base'>
              <p className='text-slate-700 font-semibold'>{accomplishment.title}</p>
              <p className='text-slate-500'>{accomplishment.description}</p>
              <p className='text-slate-400'>{accomplishment.date}</p>
            </div>
      ...
      >
        <div className='bg-white rounded-md p-4 md:p-8 w-full max-w-2xl h-full md:h-fit'>
          <div className='flex items-center gap-3'>
            ...
            <h3>{currentEditIndex !== null ? 'Edit Accomplishment' : 'Add Accomplishment'}</h3>
          </div>
          <p className='text-xs md:text-sm my-4 text-slate-500'>
            {currentEditIndex !== null ? 'Edit the details of your accomplishment below.' : 'Please provide the details of your new accomplishment.'}
          </p>
